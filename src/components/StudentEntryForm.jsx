...
const StudentEntryForm = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (response.status === 201) {
                ...
                setGeneratedToken(studId);
                setShowModal(true); // ⬅️ Show popup
    ...
    return (
        <>
            {showModal && (
                <SuccessModal token={generatedToken} onClose={() => setShowModal(false)} />
            )}
            <div className="bg-white shadow-md w-full max-w-3xl border border-gray-200 rounded-lg">
                ...
                {message && (
                    <div
                        className={`px-6 py-3 text-sm ${message.includes("✅")
