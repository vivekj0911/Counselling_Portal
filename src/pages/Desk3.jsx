...
export default function Desk3() {
  ...
  // Set the admissionStatus as true for 'approved' and false for 'rejected'
  if (name === 'admissionStatus') {
    setFormData((prev) => ({
      ...prev,
      [name]: value === "approved" ? true : value === "rejected" ? false : ""
    }));
  } else {
  ...
  const isFormValid =
  formData.admissionStatus !== "" &&  // Check if admissionStatus is not empty (true/false)
  (formData.admissionStatus === true || formData.admissionStatus === false) &&  // Validate boolean
  (formData.admissionStatus === false ? formData.reason.trim() !== "" : true);  // Only validate reason when rejected
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.put(
        ...
        {
          admissionStatus: formData.admissionStatus,  // Send true/false
    ...
    <div className="p-6 max-w-5xl mx-auto bg-white rounded-lg">
      {!studentId ? (
        ...
        <div className="max-w-xl mx-auto mt-10 bg-white shadow-xl p-6 rounded-2xl">
          <h2 className="text-2xl font-semibold text-red-700 mb-6 text-center">Desk 3 - Final Admission Decision</h2>
